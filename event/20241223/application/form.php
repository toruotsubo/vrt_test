...
// 2018/07/20 K.Nakayama v1.71 JSNイベントフォーム対応対応 個別configファイル名共通化
// 2018/09/10 K.Nakayama v1.72 Return-Path対応
// 2018/09/26 K.Nakayama v1.721 不具合修正
// 2024/11/07 K.Nakayama v1.721-utmp 対面のときカウント
//
//エラー出力(debug)
...
$conffile = "../../lib/config.php";
$customfile = "data/conf_custom.php";
...
if(isset($_POST)) {
	if(file_exists($conffile)){
		...
		if(file_exists($customfile)){
			...
			die("$customfile not exist");
		}
	}else{
		die("$conffile not exist");
	}
}else{
	die('エラー：送信データがありません');
}
//config($config); 2018/08/24
//conf_customize($config);
...
// $data : 入力値リスト
	...
	// 受付No.存在チェック
	$sql = "select count(*) from db_form_input_data where formno = :formno and id = :id";
	$param = array();
	$param[':formno'] = $config['formno'] ;
	$param[':id'] = isset($data['cancelid'])? $data['cancelid'] : null ;
	
	$count = inputDbCount($sql,$param,$config);
	if(!isset($count)) {
	// メールアドレスが無いとき
		$num_name = $r_config['num_name'] ;
		$err[$num_name] = "登録時の$num_nameを入力してください。";
	}
/* 2018/07/16
	// メールアドレス存在チェック
	$sql = "select count(*) from db_form_input_data where formno = :formno and mailadr = :mailadr";
...
// $data : 入力値リスト
	...
	if(!(isset($_POST['msgid']))||($data['msgid'] !== $_POST['msgid'])) {
		errDie('セッションエラー',1);
...
//		if (strtotime($GLOBALS['@time']) > strtotime($_POST['time'] . " +1 hour")){
	...
	foreach($items as $item) {
		...
		if(array_key_exists($name,$data)){
			...
			if(($type == FILEANY) || ($type == FILENEED)) {
				...
				if(!empty($data["$name"])) {
					...
					$fsize = cnvFilesize($data["$name" . "_fsize"]);
					if(!isset($data["$name" . "_fname"])){
						// メール・DB出力データ
						$data["$name" . "_file"] = $config['virusmsg'] . "\n ($val : $fsize)\n" ;
						$inputdata["$in_no"] = "" ;
					}else{
						$fname = $data["$name" . "_fname"] ;
		...
		//				$data["$name" . "_file"] = $config['upload_file_url'] . '/' . $fname . "\n ($val : " . ($data["$name" . "_fsize"] / 1024) . 'KB)' ;
						$data["$name" . "_file"] = $config['upload_file_url'] . '/' . $fname . "\n ($val : $fsize)" ;
						$inputdata["$in_no"] = $data["$name" . "_file"] ;
					}
				}else{
					// メール・DB出力データ
					$data["$name" . "_file"] = "" ;
			...
			// キャンセル時のキャンセル対象受付No. 2018/07/16
				if(isset($GLOBALS['@cancel']) && ($GLOBALS['@cancel'])){
					$data['cancelno'] = $inputdata["$in_no"] ; // キャンセル対象受付No.をセット-
					$inputdata['cancel'] = 1;
				}
			...
			if(isset($data["$name" . "_pay"])){
			// 支払データあり
				// 支払内容をテーブルにセット
				$param = array();
				$param[':formno'] = $config['formno'];
				$param[':id'] = $GLOBALS['@msgid'];
				$param[':countno'] = $counters['id'] ;
				$param[':input_name'] = $name ;
				$param[':items'] = $item['name'];
				$param[':price'] = str_replace(',','',$data["$name" . "_pay"]);
				
//				print_r($param);
				// sql生成
				$sql = "INSERT INTO `db_form_payments`(`formno`, `id`, `countno`, `input_name`, `items`, `price`) VALUES (:formno,:id,:countno,:input_name,:items,:price)";
	...
	// 対面時人数セット 2024/11/07
	$data['offline'] = ($data['item05']=='対面')? $counters['offline'] : $counters['offline'] - 1 ;
...
//		print_r($param);
		// sql生成
		$sql = "SELECT sum(`price`) as payment FROM `db_form_payments` WHERE `formno` = :formno and `countno` = :countno";
		dbSelectTable($config,$sql,$param,$payment);
		$inputdata['payment'] = $data['payment'] = (empty($payment))? 0 : $payment[0]['payment'] ;
		$data['payment_format'] = (empty($payment))? '0' : number_format($payment[0]['payment']) ;
	}

	// フォーム入力データテーブルにセット
	inputDbInsert($inputdata,$config);

	// キャンセルのときキャンセル対象データをupdate
	if(isset($GLOBALS['@cancel']) && ($GLOBALS['@cancel'])) {
		//キャンセル用パラメータ
		$update = array();
		$update['formno'] = $config['formno'];
		$update['canceldate'] = date("Y-m-d H:i:s"); // キャンセル日時
		$update['cancelno'] = $counters['id'] ; // キャンセル受付No.
		$update['countno'] = $data['cancelno'] ; // キャンセル受付No.をセット
		// update
		cancelDbUpdate($update,$config);
	}
	...
	//キャンセルのとき
		...
		$replybody = $config{'mail_replycancel'};
		...
		// 送信先へメール送信
		mailSend($data,$body,$config['mail_csubj'],$to,$inputdata['mailadr'],$config);
		// 返信するとき
		if(isset($reply_to)) {
			//Replyメールの返信先指定 2017/11/19
			$rep_replyto = empty($config['file_maillist'])? $config['mail_replyto'] : $data['mailtoadr'] ; // 不具合修正 2018/09/28
			// Replyメール送信
			mailSend($data,$replybody,$config['mail_replycsubj'],$reply_to,$rep_replyto,$config);
		}
	...
	//キャンセル以外のとき
	    ...
	    $cryptid = strCrypt($config['formno'] . "-" . $counters['id'] ."-" . $GLOBALS['@msgid']);
		$cancelurl = $config['form_cancel_url'] . "?mid=" . urlencode($cryptid);
	...
	//ログ出力
	$log = implode(",",$inputdata);
	logWrite('"complete : ' . $log . '"');

	if(!isset($err)) {
	//正常終了なら確認画面表示
		$data['html_thanksmsg'] = $config['html_thanksmsg'];
		$data['formname'] = $config['formname'];
		$data['title'] = $config['form_title'];
		$data['msgid'] = $GLOBALS['@msgid'];
		$data['contact'] = $config['contact'];; // 2018/08/05
		$data['contact_mail'] = $config['contact_mail']; // 2018/08/05
		session_off();//セッションオフ

		if(isset($GLOBALS['@cancel'])){
			htmlPrint('cthanks',$data);
		}else{
			htmlPrint('thanks',$data);
		}
		return(1);
	}else{
		return(0);
...
// 2024/11/07 K.Nakayama v1.721-utmp 対面のときカウント
	...
	foreach($items as $item) {
		$name = $item['input_name'];
		$type = $item['item_type'];
		if(($type == PARTY)&&(array_key_exists($name,$data))){
			$party = $data["$name"];
			break;
		}
	}
	$id = $counters['id'] + 1;
	$entry = ($GLOBALS['@cancel'])? $counters['entry'] : $counters['entry'] + $party + 1 ;// 当人含める
	...
	// 対面用カウンタ 2024/11/07
	 $param[':offline'] = ($data['item05']=='対面')? $counters['offline'] + 1 : $counters['offline'] ;
...
function cancelDbUpdate($data,$config){
	...
	// PDOパラメータセット
	$param = array();
	$param[':formno'] = $data['formno'];
	$param[':canceldate'] = $data['canceldate'];
	$param[':cancelno'] = $data['cancelno'];
	$param[':countno'] = $data['countno'];
//print_r($param); //wwwww
	// sql生成
	$sql = "update db_form_input_data set " . 
			"canceldate = :canceldate,".
			"cancelno = :cancelno " .
			"where formno = :formno and countno = :countno" ;
/*
...
function paramSet($data,&$r_param,$items_max){
	...
	$r_param[':payment'] = isset($data['payment'])? $data['payment'] : null ;
